...
from state_manager import load_total_queue_history, simulate_step, get_queue_lengths, get_queue_activity, set_queue_active
...
with col2:
    st.markdown(
        f'<div class="metric-card"><div>Predicted Wait Time (30 min)</div><div class="big-metric green-text">{predicted_wait_time_30min} <span class="wait-unit">min</span></div></div>', unsafe_allow_html=True)
with col3:
    st.markdown(
